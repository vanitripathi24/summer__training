...
import { fetchProductById, fetchRelatedProducts } from '../../services/dummyjsonApi';
...
export default function Product() {
  ...
  // Fetch the product (and related products) from the live DummyJSON API
  // whenever the route's product_id changes.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setNotFound(false);
    ...
    fetchProductById(product_id)
      .then((fetchedProduct) => {
        ...
        return fetchRelatedProducts(
          fetchedProduct.tag === 'womens' ? 'womens-shoes' : 'mens-shoes',
          fetchedProduct.id
        );
      })
      .then((relatedProducts) => {
        if (!cancelled && relatedProducts) setRelated(relatedProducts);
      })
      .catch(() => {
